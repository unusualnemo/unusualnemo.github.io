...
async function start(params, settings) {
  ...
  QuickAdd.variables = {
    ...
    Plot: selectedShow.Plot.toLowerCase(),
    Language: selectedShow.Language.toLowerCase(),
    directorLinks:
      selectedShow.Director === "N/A" ? " " : linkifyList(selectedShow.Director.split(",")).toLowerCase(),
    writerLinks:
      selectedShow.Writer === "N/A" ? " " : linkifyList(selectedShow.Writer.split(",")).toLowerCase(),
    fileName: replaceIllegalFileNameCharactersInString(selectedShow.Title).toLowerCase(),
...
function linkifyList(list) {
  ...
  return list.map((item) => `[[${item.trim()}]]`).join(", ").toLowerCase();
}
